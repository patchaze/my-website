...
const fixes = [
    { name: 'Belgium', file: '/images/destinations/belgium.jpg', title: 'Grand-Place' },
    { name: 'Portugal', file: '/images/destinations/portugal.jpg', title: 'Dom_Luís_I_Bridge' },
    { name: 'Spain', file: '/images/destinations/spain.jpg', title: 'Sagrada_Família' },
    { name: 'Sweden', file: '/images/destinations/sweden.jpg', title: 'Stockholm_Palace' },
...
async function main() {
    for (const fix of fixes) {
        ...
        if (url) {
            ...
            // fallback to country name on wikipedia if specific title fails
            const url2 = await getWikipediaImageUrl(fix.name);
            if (url2) {
                await downloadImage(url2, path.join(publicDir, fix.file));
                console.log(`✓ Fixed ${fix.name} (fallback)`);
            } else {
